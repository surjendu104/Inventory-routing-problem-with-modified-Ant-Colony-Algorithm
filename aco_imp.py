...
for i in range(customer_number+1):
    ...
    for j in range(customer_number+1):
        distance_array.append(int(distance_between_two_node(
            customer_list[i]['x'], customer_list[j]['x'], customer_list[i]['y'], customer_list[j]['y'])))
...
ideally_initial_path.append(k)
ideally_total_distance += int(distance_between_two_node(0,
                              customer_list[k]['x'], 0, customer_list[k]['y']))

'''Pheromone density(initial)'''
tau_0 = 1./ideally_total_distance
        ...
        if rev_route != i:
            try:
                ...
                sigma += (pheromone_matrix[rev_route][i]**alpha) * \
                    ((1./distance_matrix[rev_route][i])**beta)
        ...
        for i in range(customer_number):
            for j in range(customer_number):
                pheromone_matrix[i][j] += rho * \
                    pheromone_matrix[i][j]+pheromone_local[i][j]
    return dist
...
single_routing_cost += (best_distance*period)+(len(vehicle_route)
                                               * distance_matrix[best_route[0]][best_route[-1]])
